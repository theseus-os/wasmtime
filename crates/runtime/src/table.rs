...
impl Table {
    ...
    pub fn new_dynamic(plan: &TablePlan, store: &mut Option<&mut dyn Store>) -> Result<Self> {
    ...
    pub fn new_static(
        plan: &TablePlan,
        data: &'static mut [usize],
        store: &mut Option<&mut dyn Store>,
    ) -> Result<Self> {
        Self::limit_new(plan, store)?;
    ...
    fn limit_new(plan: &TablePlan, store: &mut Option<&mut dyn Store>) -> Result<()> {
